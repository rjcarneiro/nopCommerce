...
namespace Nop.Services.Localization
{
    ...
    public static class LocalizedUrlExtenstions
    {
        ...
        /// <summary>
        /// Replace seo code from url
        /// </summary>
        /// <param name="url">Raw url</param>
        /// <param name="pathBase">Application path base</param>
        /// <param name="isRawPath">A value indicating whether passed URL is raw URL</param>
        /// <param name="language">Language</param>
        /// <returns>Url with a passed language seo code</returns>
        public static string ReplaceSeoCodeInUrl(this string url, PathString pathBase, bool isRawPath, Language language = null)
        {
            if (string.IsNullOrEmpty(url) || language == null)
                return url;

            //Extract server and path from url
            var scheme = new Uri(url).GetComponents(UriComponents.SchemeAndServer, UriFormat.Unescaped);
            var path = new Uri(url).PathAndQuery;

            //Replace seo code
            var localized = path
                .RemoveLanguageSeoCodeFromUrl(pathBase, isRawPath)
                .AddLanguageSeoCodeToUrl(pathBase, isRawPath, language);

            return new Uri(new Uri(scheme), localized).ToString();
        }
